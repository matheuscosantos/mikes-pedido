...
resource "aws_iam_policy" "ecs_execution_role_additional_policy" {
  name   = "${var.name}_ecs_execution_role_additional_policy"
  policy = file("iam/policy/ecs_execution_role_additional_policy.json")
}

resource "aws_iam_policy_attachment" "ecs_execution_role_additional_policy_attachment" {
  name       = "${var.name}_ecs_execution_role_additional_policy_attachment"
  roles      = [aws_iam_role.ecs_execution_role.name]
  policy_arn = aws_iam_policy.ecs_execution_role_additional_policy.arn
}
...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  ...
  container_definitions = templatefile("container/definitions/mikes_app_container_definitions.json", {
    ...
    SNS_ORDER_CONFIRMED_NAME    = aws_sns_topic.sns_topic_pedido_confirmado.arn
    REGION                      = var.region
