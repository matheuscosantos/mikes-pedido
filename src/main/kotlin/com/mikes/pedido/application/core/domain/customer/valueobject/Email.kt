...
value class Email private constructor(val value: String) {
    companion object {
        ...
        private fun String.validated(): Result<String> {
            if (isEmpty()) {
                return failure(InvalidEmailException("empty email."))
            }
            if (isTooBig()) {
                return failure(InvalidEmailException("email cannot be bigger than $MAX_LENGTH."))
            }
            if (!isValidPattern()) {
                return failure(InvalidEmailException("invalid email."))
            }
